...
(function(BasicGame) {
    ...
    BasicGame.TextAndSpeechWithinGame = function () {
    ...
    BasicGame.TextAndSpeechWithinGame.prototype = {
        ...
        create: function () {
			...
			this.numberOfAudioFilesToLoad = 0;
		...
		goBackToGame: function() {
			...
			if ((this.oldLang != currentLang))
			{
				...
				/* Workout if we need to load any audio START */
				var newAudioRequired = false;
				...
				if ((newAudioRequired == true))
				{
					...
					for (i = 0; i < gameData.rooms.length; i++) {
						if ((gameData.rooms[i].id == currentRoom)) {
							...
							for (x = 0; x < localisation.languages.length; x++) {
								if ((localisation.languages[x].id == currentLang) && (localisation.languages[x].allowNarrationAudio == true)) {
									if ((this.game.cache.checkSoundKey(gameData.rooms[i].narration.languages[currentLang].narrationId) == false)) {
										this.numberOfAudioFilesToLoad = this.numberOfAudioFilesToLoad + 1;
										var newNarrationToLoad = new Phaser.Loader(game);
										newNarrationToLoad.audio(gameData.rooms[i].narration.languages[currentLang].narrationId, ["" + gameData.rooms[i].narration.languages[currentLang].narrationId + ".mp3", "" + gameData.rooms[i].narration.languages[currentLang].narrationId + ".ogg"]);
										newNarrationToLoad.onLoadComplete.add(this.deductFromCounter, this, this);
										newNarrationToLoad.start();
									}
								}
							}
						}
					}
					/* Load in new narration END */
					...
					for (i = 0; i < localisation.languages.length; i++)
					{
						if ((localisation.languages[i].id == currentLang) && (localisation.languages[i].allowNarrationAudio == true) && (globalSpeeches.languages[currentLang].id != null))
						{
							if ((this.game.cache.checkSoundKey(globalSpeeches.languages[currentLang].id) == false)) {
								...
								newGlobalNarrationToLoad.onLoadComplete.add(this.deductFromCounter, this, this);
				...
				/* Workout if we need to load any audio END */


			}
			/* If the language has been changed, remove the old language narration from the current room and load in the new one END */
			/* No change detected so just go back as normal START */
			else
			{
				this.state.start('OptionsWithinGame');
			}
			/* No change detected so just go back as normal END */
		...
		deductFromCounter: function(){
			this.numberOfAudioFilesToLoad = this.numberOfAudioFilesToLoad - 1;

			if ((this.numberOfAudioFilesToLoad == 0))
			{
				this.state.start('OptionsWithinGame');
			}
		},
